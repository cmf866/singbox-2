...
# 需要修改的点： 22-28行，就改-后面的引号里的值即可。
# argo_auth值的获取：步骤1、登录cf网站。步骤2、浏览器打开https://fscarmen.cloudflare.now.cc/    步骤3、在新打开的步骤2的网址界面中的Cloudflare services里面的tunnel 1填写你的argo域名的前缀，比如ag.xxx.us.kg,就填写 ag，然后点击submit按钮
# 步骤4、在submit按钮提交后，正下方会出现Cloudflare Authorize的链接，点击后会自动新开一个浏览器窗口，然后需要你去选择你要授权的域名。完成授权操作。
# 步骤5、Cloudflare Authorize正下方的那个大输出框里会出现一段代码，第一行为你的argo的域名，剩下那个大括号就是你的argo_auth值。
...
check_process_alive(){
 ALIVE_COUNT=$(sockstat -l | grep  "${VMESS_PORT}\|${HY2_PORT}\|${TUIC_PORT}" |wc -l)


 if [[ $ALIVE_COUNT -ne 3 ]] 
 then
   purple "some tcp or udp processes  not alive, process starting... "
   ps aux | grep $(whoami) | grep -v "sshd\|bash\|grep" | awk '{print $2}' | xargs -r kill -9 2>/dev/null
 else
   yellow "processes already exists, not need to restart process again! "
   return 1 
 fi
